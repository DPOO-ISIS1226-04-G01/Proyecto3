...
public class LoginEmpl extends JFrame implements ActionListener{
	...
	MenuEmpleado menu;
	TipoUsuario vAnterior;
	JLabel textoLogin;
	JButton iniciar,salir;
	JTextField campoLogin,campoPassword;
	Aplicacion consola1;
	
	public LoginEmpl(Aplicacion consola12) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		}else if (comando.equals(INICIAR) && consola1.iniciarEmpleadoInterfaz(campoLogin.getText(),campoPassword.getText())) {
			menu = new MenuEmpleado(consola1);
			menu.setVisible(true);
			setVisible(false);
		}else if (comando.equals(INICIAR) && (consola1.iniciarEmpleadoInterfaz(campoLogin.getText(),campoPassword.getText()) == false)) {
			JOptionPane.showMessageDialog(null,"Usuario y/o contrasena incorrectos");
		}
		if(comando.equals(SALIR)) {
			vAnterior = new TipoUsuario(consola1);
